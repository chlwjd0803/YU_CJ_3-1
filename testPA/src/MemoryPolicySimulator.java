...
public class MemoryPolicySimulator {
    ...
    public static final class Core {
            ...
            if(isHit){
                ...
                if(frame.size()>=cap){
                    char victim = switch(policy){
                        ...
                        case "OptiLRU" -> { char v=optlruVictim(ref,idx); frame.remove(v); recent.remove(v); yield v; }
        ...
        private static boolean isLRU(String p){ return "LRU".equals(p)||"OptiLRU".equals(p); }
        private static int indexOf(Deque<Character> dq,char t){ int i=0; for(char c:dq){ if(c==t) return i; i++; } return -1; }
        private char optimalVictim(char[] ref,int idx){
            int far=-1; char vic=frame.peekFirst();
            for(char p:frame){ int nxt=ref.length; for(int j=idx+1;j<ref.length;j++){ if(ref[j]==p){ nxt=j; break; } } if(nxt>far){ far=nxt; vic=p; } }
            return vic;
        }
    ...
    private static final class MainFrame extends JFrame{
        private final JTextField tfRef=new JTextField(30);
        private final JTextField tfCap=new JTextField("4",4);
        private final JComboBox<String> cbPol=new JComboBox<>(new String[]{"FIFO","LRU","Optimal","OptiLRU"});
        ...
        private JPanel buildControls(){
            ...
            run.addActionListener(e->runSim()); rnd.addActionListener(e->tfRef.setText(randomRef())); save.addActionListener(e->timeline.exportPNG(this));
            JPanel p=new JPanel();
        ...
        private String randomRef(){ Random r=new Random(); int len=r.nextInt(45)+5; StringBuilder sb=new StringBuilder(); for(int i=0;i<len;i++) sb.append((char)('A'+r.nextInt(26))); return sb.toString(); }
        private void runSim(){
            log.setText(""); String refs=tfRef.getText().trim().toUpperCase(); if(refs.isEmpty()) return;
            int cap; try{ cap=Integer.parseInt(tfCap.getText().trim()); }catch(NumberFormatException ex){ return; }
            Core core=new Core(cap); char[] arr=refs.toCharArray();
            for(int i=0;i<arr.length;i++){ core.operate(arr[i],cbPol.getSelectedItem().toString(),arr,i); log.append("데이터 "+arr[i]+" → "+core.hist.get(i).status+"\n"); }
    ...
    private static final class PiePanel extends JPanel{
        ...
        @Override protected void paintComponent(Graphics g){
            ...
            int d=Math.min(getWidth(),getHeight())-80; int cx=30, cy=10;
